...
graph TD
    ...
    DetectIntent -->|聊天意图| ChatLLM[M9.5: 通用聊天接口<br/>不使用SQL模板<br/>直接对话]
    DetectIntent -->|查询意图| DetectMultiTable{M8: 检测多表查询<br/>识别涉及的表}
    
    ChatLLM --> ChatResponse2[M9.5: 保存聊天回复<br/>is_chat_response=True]
    ...
    DetectChat -->|是聊天响应| ChatResponse[M9.5: 使用通用聊天接口<br/>不使用SQL模板<br/>直接返回LLM回复]
    DetectChat -->|是SQL查询| Clarify[澄清判断<br/>clarify_node<br/>M7]
    
    ChatResponse --> AnswerBuilder
    ...
    StoreAnswer --> EchoResult[输出结果<br/>echo_node<br/>M9: 显示自然语言答案]
    Fail --> EchoResult
